...
> 
> - **2026.05.25 (v3.1.0)** - **Kiro 0.11→0.12 プロトコル移行と安定性強化**。Kiro 0.12 プロトコルの三段階移行を完了し、キャッシュと思考チェーンの基準を統一。ストリーム retry-wait 中に SSE キープアライブハートビートを追加し、`write after end` および中間プロキシの 524 タイムアウトを回避。`x-claude-code-session-id` ヘッダー解析を追加し Kiro 上流 `conversationId` に転送。Health-Check の 403/502/429 修正（プロキシセッションローテーション + in-flight refresh の重複排除 + 429 クールダウン）。Write/Edit/Agent ツール説明の切り出しホワイトリスト。複数のツール解析修正（Write file_path/content の欠落、不完全ツール呼び出し判定）。
