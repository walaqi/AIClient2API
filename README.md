...
> 
> - **2026.05.25 (v3.1.0)** - **Kiro 0.11→0.12 Protocol Migration & Stability Hardening**. Completed three-phase migration to Kiro 0.12 protocol with cache/thinking-chain unification; added SSE keep-alive heartbeat during retry-wait to prevent `write after end` and intermediate-proxy 524 timeouts; new `x-claude-code-session-id` header forwarding to upstream Kiro `conversationId`; Health-Check 403/502/429 fixes with proxy session rotation, in-flight refresh dedup, and 429 cooldown; Write/Edit/Agent tool description whitelist; multiple tool-parsing fixes (Write file_path/content drop, incomplete-tool-call detection).
